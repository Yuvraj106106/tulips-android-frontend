    ...
    PanResponder.create({
      ...
      onPanResponderMove: (evt) => {
        ...
        if (activeRing === 'year') {
          ...
          const newMonth = Math.floor((normalizedAngle / 360) * 12);
          setMonth(Math.min(11, Math.max(0, newMonth)));
        } else if (activeRing === 'day') {
          const newDay = Math.floor((normalizedAngle / 360) * 31) + 1;
          setDay(Math.min(31, Math.max(1, newDay)));
        }
      },
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.dialContainer} {...panResponder.panHandlers}>
        <Svg width={DIAL_SIZE} height={DIAL_SIZE} viewBox={`0 0 ${DIAL_SIZE} ${DIAL_SIZE}`}>
          ...
          <G transform={`rotate(${(day - 1) / 31 * 360}, ${CENTER}, ${CENTER})`}>
